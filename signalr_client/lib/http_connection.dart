...
class NegotiateResponse {
  ...
  NegotiateResponse.fromJson(Map<String, dynamic> json)
      : assert(json != null),
        this.connectionId = json['connectionId'] as String,
    ...
    for (var i = 0; i < transports.length; i++) {
      availableTransports.add(
          AvailableTransport.fromJson(transports[i] as Map<String, dynamic>));
    }
  }
}
...
class AvailableTransport {
  ...
  AvailableTransport.fromJson(Map<String, dynamic> json) {
    ...
    transport = httpTransportTypeFromString(json['transport'] as String);
    List<dynamic> formats = json['transferFormats'] as List<dynamic>;
    if (formats == null) {
      return;
    }
    for (var i = 0; i < formats.length; i++) {
      transferFormats.add(getTransferFormatFromString(formats[i] as String));
    }
  }
}
...
class HttpConnection implements IConnection {
  ...
  static ConnectionSignalState connectionState;
  String _baseUrl;
  ...
  HttpConnection(String url, {HttpConnectionOptions options})
    ...
    connectionState = ConnectionSignalState.Disconnected;
  }
  ...
  Future<void> start({TransferFormat transferFormat}) {
    ...
    if (connectionState != ConnectionSignalState.Disconnected) {
      return Future.error(GeneralError(
          "Cannot start a connection that is not in the 'Disconnected' state."));
    }

    connectionState = ConnectionSignalState.Connecting;
  ...
  Future<void> send(Object data) {
    if (connectionState != ConnectionSignalState.Connected) {
      return Future.error(GeneralError(
  ...
  Future<void> _startInternal(TransferFormat transferFormat) async {
    ...
    try {
      ...
      _changeState(ConnectionSignalState.Connecting, ConnectionSignalState.Connected);
    } catch (e) {
  ...
  Future<NegotiateResponse> _getNegotiationResponse(String url) async {
    ...
    try {
      ...
      return NegotiateResponse.fromJson(
          json.decode(response.content as String) as Map<String, dynamic>);
    } catch (e) {
      _logger?.severe(
  ...
  bool _changeState(ConnectionSignalState from, ConnectionSignalState to) {
